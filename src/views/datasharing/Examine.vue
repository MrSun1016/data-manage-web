  ...
  <div id="examine">
    <el-card class="box-card" v-show="!isshowHaredetails">
      ...
      >
        ...
        <div class="second" style="display: flex; justify-content: space-between" v-show="isSearch">
          ...
          <el-form-item label="数据发起时间：" style="width: 330px">
            <el-date-picker
              @change="onInquire"
        ...
        <div style="display: flex; justify-content: space-between; flex-direction: column;">
          <el-form-item label="申请部门：" style="width: 330px" v-show="isSearch">
            <el-select
              ref="sourceUnit"
              @keydown.enter.native="disableVisible"
              clearable
              filterable
              class="items"
              v-model="tabsOptions.inquireData.sourceUnit"
              size="small"
              placeholder="请选择"
            >
              <el-option
    ...
    <div
      class="box-card"
      style="margin-top: 24px; box-shadow: 0 2px 12px 0 rgb(0 0 0 / 10%)"
      :class="isshowHaredetails ? 'trueCss' : 'falseCss'"
    >
      <InformationExamine ref="exatablesLoading" @handleShow="handleShow" :informationData="informationData" />
...
export default {
  ...
  methods: {
    ...
    initShareDataPage() {
      ...
      getshareDataPage(this.tabsOptions.inquireData).then((res) => {
        if (res.success) {
          this.$refs.exatablesLoading.tableLoading = false
          this.informationData = res.body.content
          this.handleEllipsis()
    ...
    onReset() {
      ;(this.tabsOptions.inquireData.useAppSystem = ''),
        ...
        (this.tabsOptions.inquireData.sourceUnit = '')
      //   this.onInquire()
    },
